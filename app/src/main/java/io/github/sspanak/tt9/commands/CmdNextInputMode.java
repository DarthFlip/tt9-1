...
public class CmdNextInputMode implements Command {
	...
	public boolean runFromHotkey(@Nullable TraditionalT9 tt9, boolean validateOnly) {
		...
		// Fork affordance: when triggered by the # hotkey (not the mode soft-key), first get the
		// on-screen QWERTY panel out of the way so the T9 keypad workflow takes over.
		tt9.collapseQwertyKeysForModeSwitch();
		return run(tt9);
	}
